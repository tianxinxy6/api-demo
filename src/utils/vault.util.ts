...
export class VaultUtil {
    ...
    private async initializeVault(): Promise<void> {
        try {
            ...
            if (healthResponse.status === 200) {
                ...
                this.logger.warn(
                    `Vault health check returned status ${healthResponse.status}`,
                );
            }
        } catch (error) {
        ...
        try {
            if (!this.config.roleId || !this.config.secretId) {
                throw new BadRequestException(
                    'Vault AppRole credentials (roleId/secretId) not configured',
                );
            }
            ...
            this.logger.debug(
                `Got new Vault token, expires in ${authData.lease_duration}s`,
            );
    ...
    async storePrivateKey(
        ...
        try {
            ...
            await firstValueFrom(
                this.httpService.post(
                    `${this.config.address}/v1/${secretPath}`,
                    payload,
                    {
                        headers: {
                            'X-Vault-Token': token,
            ...
            this.logger.error(
                `Failed to store private key in Vault: ${error.message}`,
            );
    ...
    async getPrivateKey(walletId: string): Promise<PrivateKeyData | null> {
        try {
            ...
            this.logger.error(
                `Failed to retrieve private key from Vault: ${error.message}`,
            );
